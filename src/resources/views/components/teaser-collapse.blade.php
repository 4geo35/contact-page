...
<div class="h-full mb-indent-half">
    ...
    <x-tt::h4 class="mb-indent-half cursor-pointer hover:text-body/40"
              @click="$dispatch('move-to', { el: $refs.contact{{ $contact->id }} }); current = {{ $contact->id }}; document.getElementById('map').scrollIntoView()">
        {{ $contact->title }}
    </x-tt::h4>
    @if ($contact->address)
    ...
    <div x-show="current === {{ $contact->id }}" x-collapse style="display: none">
